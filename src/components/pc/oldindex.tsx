...
import { Feedback, MineSweeperGame } from '../apps/appExports'
    ...
    return (
      ...
      >
        ...
        {(isConnected || isLoginPregenSession) && (
          <div className="absolute flex pr-7 border border-red-800 flex-row-reverse justify-start gap-4 top-0 w-full bg-gradient-to-r p-2 to-[#0a246a] from-[#2563eb] h-10">
            <div>
              <button
                onClick={() => {
                  openNotificationPanel()
                }}
                className="rounded-full border border-red-800 text-white  shadow-lg hover:text-gray-400"
              >
                <Bell size={20} />
                {unreadCount > 0 && (
            ...
            <div className="flex border border-red-800 w-fit justify-end items-center h-full px-2">
              <TimeWidget />
            </div>
